...
if (implementation !== "npm" && implementation !== "lil") {
...
if (implementation === "npm") {
  ...
  $ = (await import(`${pathToFileURL(resolve(root, "dist/jquery.esm.js")).href}?b=${process.pid}`))
    .jQuery
}
...
function coreWork(count, retain) {
  ...
  for (let iteration = 0; iteration < count; iteration += 1) {
    ...
    const selected = $(fixture)
      .find(`.g-${iteration & 7} .item`)
      .filter(iteration & 1 ? ".odd" : ".even")
